...
module.exports = {
  name: "react",
  description: "Reacts to a user's messages with the specified emoji. (Requires Nitro for animated emojis)",
  ...
  execute(message, args) {
    ...
    try {
      var tempNamespace = {};
      tempNamespace[firstMention.id] = message.guild.roles.cache.find(role => role.name === pushDataToJSON(firstMention.id, args3));
      if (args3 != 0) {
        message.channel.send("User " + firstMember.displayName + "'s messages will now be reacted to with " + args3);
      }
    } catch(error) {
      console.error(error);
      message.channel.send("An Error Occured!");
